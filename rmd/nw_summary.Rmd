...
for (i in window_sizes) {
  ...
  ms_tw <- msfile %>%
    group_by(topology) %>%
    summarise(sumlen = sum(length), percentage = sumlen/sum(msfile$length) * 100)
  
  sm_tw <- smfile %>%
    group_by(newick) %>%
    summarise(sumlen = sum(length), percentage = sumlen/sum(smfile$length) * 100)
  
  cmptw <- c()
  for (j in 1:nrow(ms_tw)) {
    top_idx <- match(ms_tw$topology[j], sm_tw$newick)
    if (is.na(top_idx)) {
