  ...
  for (j in mstree_uq) {
    for (k in smtree_uq) {
      if (ape::all.equal.phylo(ape::unroot(ape::read.tree(text = j)), ape::read.tree(text = k))) {
        smfile[, newick := gsub(k, j, newick, fixed = TRUE)]
        break
      }
    }
  }
...
for (i in window_sizes) {
  ...
  caic <- ifelse(!file.exists(fn_contiguous), NA, gsub("^.* ", "", system(paste("grep '^Akaike information criterion'", fn_contiguous), intern = T)))
  caicc <- ifelse(!file.exists(fn_contiguous), NA, gsub("^.* ", "", system(paste("grep '^Corrected Akaike information criterion'",fn_contiguous), intern = T)))
  cbic <- ifelse(!file.exists(fn_contiguous), NA, gsub("^.* ", "", system(paste("grep '^Bayesian information criterion'",fn_contiguous), intern = T)))
