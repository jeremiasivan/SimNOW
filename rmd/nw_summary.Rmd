...
fn_sumfig <- paste(window_outdir, params$prefix, ".sum.tiff", sep="")
```
...
# correlation plot
tdata <- data.frame()
for (r in 1:nrow(t_accuracy)) {
  for (c in 3:ncol(t_accuracy)) {
    tdata <- rbind(tdata, c(t_accuracy[r,2],t_accuracy[r,c],colnames(t_accuracy)[c]))
  }
}

colnames(tdata) <- c("accuracy","value","type")

tiff(filename=fn_sumfig, units="px", width=2880, height=1800)
print(ggplot(tdata, aes(y=as.numeric(accuracy), x=as.numeric(value), ymin=0, ymax=100)) + 
