...
infocr$window_size <- as.character(infocr$window_size)
infocr$r <- as.character(infocr$r)
...
for (i in 5:ncol(infocr)-1) {
  ...
  tiff(filename=fn_simfig, units="px", width=2880, height=1800)
  print(ggplot(infocr, aes(y=accuracy, x=as.numeric(unlist(infocr[,..i])), ymin=0, ymax=100, color=r)) +
          geom_jitter(aes(shape=window_size), width = 0, height = 0, size=5) +
