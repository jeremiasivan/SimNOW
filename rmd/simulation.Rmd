...
if (file.exists(simulation_output)) {
  print("msfile already exists. Skipping this step...")
} else {
  print(paste(params$msdir,ms_command,">",simulation_output))
  system(paste(params$msdir,ms_command,">",simulation_output))
}
```

```{r postms, echo = FALSE, message = FALSE, collapse = TRUE}
simulation_outdir <- paste(getwd(), "/simulation", sep="")
...
if (file.exists(fn_mstrees) && file.exists(fn_mstops) && file.exists(fn_mttrees) && file.exists(fn_uqtops)) {
...
if (file.exists(paste(fn_alisim, ".fa", sep = ""))) {
  print("AliSim simulated alignment already exists. Skipping this step...")
} else {
  if (params$alisim_scale == 0 || is.null(params$alisim_scale)) {
    print(paste(params$iqtree2dir,"--alisim",fn_alisim,"-Q",fn_nexus,"-t",fn_mttrees,"-m",params$alisim_model,"-af fasta", sep=" "))
    system(paste(params$iqtree2dir,"--alisim",fn_alisim,"-Q",fn_nexus,"-t",fn_mttrees,"-m",params$alisim_model,"-af fasta", sep=" "))
  } else {
    print(paste(params$iqtree2dir,"--alisim",fn_alisim,"-Q",fn_nexus,"-t",fn_mttrees,"--branch-scale",params$alisim_scale,"-m",params$alisim_model,"-af fasta", sep=" "))
