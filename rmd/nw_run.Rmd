...
# create doParallel cluster
nwcl <- parallel::makeCluster(params$thread)
doParallel::registerDoParallel(nwcl)
...
if (length(ls_seqlen) > 0) {
  for (i in 1:length(ls_seqlen)) {
    ...
    temp_outdir <- paste(window_outdir,ws,"/summary/",sep="")
    fn_atsum <- paste(temp_outdir, params$prefix, ".atsum", sep="")
    fn_atsumf <- paste(temp_outdir, params$prefix, ".atsum.tiff", sep="")
    fn_cnsum <- paste(temp_outdir, params$prefix, ".cnsum", sep="")
    fn_cnsumf <- paste(temp_outdir, params$prefix, ".cnsum.tiff", sep="")
    ...
    write.table(ls_seqlen[[i]][[2]], file=fn_atsum, quote=FALSE, sep="\t", row.names=F, col.names=T)
    write.table(ls_seqlen[[i]][[4]], file=fn_tops, quote=FALSE, sep="\t", row.names=F, col.names=T)
    write.table(ls_seqlen[[i]][[5]], file=fn_cnsum, quote=FALSE, sep="\t", row.names=F, col.names=T)
    ...
    log4r::info(fn_logger, paste0("Files created: ", ws, "bp summary files", sep=""))
  }
}
...
```{r nowctalignment, include=FALSE}
s <- read.fasta(fn_alisim, whole.header = T)
sdf <- do.call(rbind,s)
...
for (i in window_sizes) {
  ...
  fn_map <- paste(temp_outdir, "../", params$prefix, ".map", sep="")
  acount <- length(list.files(temp_outdir,pattern='fa'))
  if (file.exists(fn_map) && acount != 0 && !params$redo) {
    log4r::info(fn_logger, paste0("Files already exist: ", i, "bp contiguous window alignment and map. Skipped.", sep=""))
    next
  }
  ...
  write.table(newick_map, fn_map, quote = F, row.names = F, col.names = F)
...
foreach (i = window_sizes, .errorhandling = 'pass') %dopar% {
  ...
  if (tcount != 0 && !params$redo) {
    log4r::info(fn_logger, paste0("Files already exist: ", i, "bp contiguous window treefile. Skipped.", sep=""))
    next
  }
