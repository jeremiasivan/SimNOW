...
if (params$filter_gaps) {
  params$ms_l <- seqinr::getLength(s)[1]
  sfactors <- getFactors(params$ms_l, params$min_window_size)
  
  indices <- seq(1, length(sfactors), length.out = params$window_len)
  params$window_size <- sort(as.numeric(sfactors[indices]))
}
...
for (i in window_sizes) {
  ...
  if (params$set_model){
    iqtree_cmd <- paste(iqtree_cmd, "-m", params$alisim_model)
  }
