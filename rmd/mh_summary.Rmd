...
for (i in mast_dirs) {
  ...
  if (all(file.exists(fn_hmmtw, fn_hmmsum, fn_hmmfig)) && params$redo == FALSE) {
  ...
  log4r::info(fn_logger, paste0("Files created: set ", i, " ", params$analysis_type, " summary files (",
                               params$prefix, ".hmmtw, ", params$prefix, ".hmmsum and ", params$prefix, ".hmmsum.tiff)."))
  
  rm(hmmfile, hmmtrees, tmfile, ms_tw, trees, dat, empty_matrix, output, sub_output)
}
