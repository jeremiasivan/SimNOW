...
fn_mast_tw <- paste0(mast_sumdir, params$prefix, ".mast_tw")
fn_mast_sum <- paste0(mast_sumdir, params$prefix, ".mast_sum")
fn_hmmsum <- paste0(mast_outdir, params$prefix, ".hmmsum")
fn_summary <- paste0(mast_outdir, params$prefix, ".mh.sum")
```

```{r summast, include=FALSE}
if (all(file.exists(fn_mast_tw, fn_mast_sum)) &&!params$redo){
  log4r::info(fn_logger, paste("File found:", toupper(mast_analysis), "summary files."))
  ...
  if (mast_analysis == MAST) {
    ...
    for (j in 1:nrow(ms_tw)) {
      top_idx <- match(ms_tw$topology[j], uq_tmtrees)
      output <- rbindlist(list(output, list(ms_tw$topology[j], ms_tw$ms[j], round(as.numeric(ls_mast_tw[top_idx]), 4), round(as.numeric(ls_hmm_tw[top_idx]), 4))))
    }
  }
  
  data.table::fwrite(output, fn_mast_tw, quote = F, sep = "\t")
  ...
  rm(msfile, ms_tw, uq_mstrees, uq_tmtrees)
}
```
