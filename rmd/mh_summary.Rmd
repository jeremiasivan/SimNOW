...
for (i in mast_dirs) {
  ...
  fn_mastsum <- paste(outdir, params$prefix, ".mastsum", sep="")
  fn_hmmsum <- paste(outdir, params$prefix, ".hmmsum", sep="")
  fn_hmmfig <- paste(outdir, params$prefix, ".hmmsum.tiff", sep="")
  if (all(file.exists(fn_mastsum, fn_hmmsum, fn_hmmfig)) && params$redo == FALSE) {
  ...
  # tree weights comparison with ms topology
  ms_tw <- msfile %>%
    group_by(topology) %>%
  ...
  ms_tw <- cbind(ms_tw, mast=mtw)
  write.table(ms_tw, file=fn_mastsum, quote = F, row.names = F)
  ...
  log4r::info(fn_logger, paste("Files created: set ", i, "MAST-HMM summary files (",
                               params$prefix, ".mastsum, ", params$prefix, ".hmmsum and ", params$prefix, ".hmmsum.tiff).", sep=""))
}
```
...
for (c in 3:ncol(t_accuracy)) {
  ...
  print(ggplot(t_accuracy, aes(y=as.numeric(accuracy), x=as.numeric(t_accuracy[,c]), ymin=0, ymax=100)) + 
          ...
          ggtitle("Correlation of MAST-HMM Information Criterion and NOW Accuracy") + 
...
log4r::info(fn_logger, paste0("Files created/modified: NOW accuracy vs MASTHMM IC correlation plots.", sep=""))
```