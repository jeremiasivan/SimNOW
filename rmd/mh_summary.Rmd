...
mast_dirs <- mast_dirs[mast_dirs != "summary"]
if (!length(mast_dirs)) {
  log4r::error(fn_logger, "Error: HMM directory is not found. Exited.")
  knitr::knit_exit()
}
...
for (i in mast_dirs) {
  ...
  rm(hmmtrees, ms_tw, trees, output, uq_mstrees, uq_tmtrees)
}
...
for (i in mast_dirs) {
  ...
  rm(hmmsum, acc, aic, aicc, bic, hmm)
}
