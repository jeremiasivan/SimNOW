...
if (mast_analysis == MAST) {
  ...
  # add weights based on ms topologies
  for (j in 1:nrow(ms_tw)) {
    top_idx <- match(ms_tw$topology[j], uq_tmtrees)
  ...
  # add weights based on MAST topologies
  for (j in subset(uq_tmtrees, !uq_tmtrees %in% uq_mstrees)) {
    top_idx <- match(j, uq_tmtrees)
    
    output <- rbindlist(list(output, list(j,
                                        "NA",
                                        round(as.numeric(ls_mast_tw[top_idx]), 4),
                                        round(as.numeric(ls_hmm_tw[top_idx]), 4))))
  }
...
if (mast_analysis == HiMAST) {
  ...
  if (!file.exists(fn_hmmms)) {
    log4r::error(fn_logger, "Error: ms-based HMM file is not found. Skipped.")
