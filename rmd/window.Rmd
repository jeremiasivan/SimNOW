...
```{r initialization, echo = FALSE, collapse = TRUE}
...
for (w in params$window_size) {
  ...
  if (sapply(temp_outdir,function(dir){length(list.files(dir,pattern='fa'))}) != 0) {
...
```{r windowtree, echo = FALSE, collapse = TRUE}
window_outdir <- paste(getwd(), "/windows/", sep="")
...
for (i in window_sizes) {
  ...
  if (sapply(temp_outdir,function(dir){length(list.files(dir,pattern='treefile'))}) != 0) {
    ...
    if (is.null(params$outgroup) || params$outgroup == ""){
      system(paste(params$iqtree2dir,"-S",fastadir,"-m",params$alisim_model,"-pre",prefix,"--quiet -T 2", sep=" "))
    } else {
      system(paste(params$iqtree2dir,"-S",fastadir,"-m",params$alisim_model,"-pre",prefix,"--quiet -T 2 -o",params$outgroup, sep=" "))
    }
  }
}
...
```{r annotation, echo = FALSE, message = FALSE, collapse = TRUE}
window_outdir <- paste(getwd(), "/windows/", sep="")
window_sizes <- list.dirs(window_outdir, full.names = F, recursive = F)
...
for (i in window_sizes) {
  ...
  if (file.exists(fn_summary) && file.exists(fn_figure) && file.exists(fn_count) && file.exists(fn_cfigure) && file.exists(fn_tops)) {
...
```{r contiguous, echo = FALSE, collapse = TRUE}
window_outdir <- paste(getwd(), "/windows/", sep="")
window_sizes <- list.dirs(window_outdir, full.names = F, recursive = F)
