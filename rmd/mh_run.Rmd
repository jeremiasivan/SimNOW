...
for (k in 1:nrow(uq_tops)) {
  ...
  if (file.exists(paste(prefix,".hmm",sep="")) && params$redo == FALSE) {
    print(paste(uq_tops$wsizes[k], "bp window MAST-HMM file already exists. Moving on..."))
    next
  }
...
for (i in 1:nrow(uq_tops)) {
  ...
  if (all(file.exists(fn_hmmsum, fn_hmmfig)) && params$redo == FALSE) {
    print("MAST-HMM summary file already exists. Moving on...")
    next
  }
  ...
  # convert rooted tree to unrooted tree
  ...
  for (k in 1:nrow(msfile)){
    ...
    # count number of sites that recovers the ms topology
    sites_sum <- sites %>%
      group_by(tree) %>%
      summarise(n=n())
