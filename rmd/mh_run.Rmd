...
fn_topset <- paste(mast_outdir, params$prefix, ".topset")
...
write.table(uq_tops, file=fn_topset, quote=F, row.names=F, col.names=F)
```
...
for (k in 1:nrow(uq_tops)) {
  ...
  outdir <- paste(setfn, tolower(gsub("[[:punct:]]","_",mast_model)), "/", sep="")
