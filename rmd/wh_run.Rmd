...
for (i in 1:p) {
  ...
  if (file.exists(fn_current_fasta)) {
    ...
    if (i == p || current_topology == atsum$topology[i+1]) {
      ...
      run_himast(fn_alignment, fn_topology, 1/seqlength, mast_model, params$iqtree2dir)
...
write.table(c("",
              ...
              paste("Changes in accuracy:", acc - wsize$accuracy[1])),
              file=fn_log, quote=F, row.names=F, col.names=F, append=T)
```
...
# compare IC value
col_idx <- match(ic_type, colnames(sum))
fn_iqtree <- paste0(prefix, ".iqtree")

now_ic <- as.numeric(sum[1,..col_idx])
hmm_ic <- 0
...
write.table(c("",
              ...
              paste("Changes in IC:", hmm_ic - now_ic)),
              file=fn_log, quote=F, row.names=F, col.names=F, append=T)
```