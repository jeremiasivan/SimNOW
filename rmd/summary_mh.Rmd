---
title: "Summary of MAST-HMM and HMMSTER Analyses"
params:
  prefix: ""
  outdir: ""
  redo: TRUE
...
hmmsumdir <- paste0(params$outdir, "/simsum/", tolower(params$analysis_type), "/")
...
hmmdir <- paste0(params$outdir, "/", simulation[1], "/", tolower(params$analysis_type), "/")
top_sets <- list.dirs(hmmdir, full.names = F, recursive = F)
model_list <- list.dirs(paste0(hmmdir, top_sets[1], "/"), full.names = F, recursive = F)
...
for (s in simulation) {
  ...
  for (r in 1:nrow(dftmp)) {
    dfsets <- rbind(dfsets, c(dftmp$wsizes[r], r, s, command[[1]][length(command[[1]])-1]))
  }
}
