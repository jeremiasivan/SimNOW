...
if (all(file.exists(fn_hmmacc, fn_hmmtopset, fn_hmmfig, fn_hmmwacc, fn_hmmwfig)) && params$redo == FALSE) {
  print("MAST-HMM summary files already exist. Exited.")
  knitr::knit_exit()
}
...
top_sets <- list.dirs(paste(params$outdir, "/", simulation[1], "/", tolower(params$analysis_type), "/", sep=""), full.names = F, recursive = F)
model_list <- list.dirs(paste(params$outdir, "/", simulation[1], "/", tolower(params$analysis_type), "/", top_sets[1], "/", sep=""), full.names = F, recursive = F)
...
for (e in 1:nrow(dfsets)) {
  ...
  mhtmp <- paste(params$outdir, "/", dfsets$simulation[e], "/", tolower(params$analysis_type), "/", dfsets$set[e], "/", sep="")
