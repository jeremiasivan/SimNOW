...
# generate seedms
if (file.exists("seedms") && params$redo == FALSE) {
...
# generate msfile
if (file.exists(fn_msfile) && params$redo == FALSE) {
  log4r::info(fn_logger, "File already exists: msfile. Skipped.")
} else {
  system(paste(params$msdir,ms_command,">",fn_msfile))
  log4r::info(fn_logger, paste("File created: ", params$prefix, ".ms.", sep=""))
}
```
...
if (all(file.exists(fn_mstrees, fn_mttrees, fn_mstops, fn_uqtops)) && params$redo == FALSE) {
  ...
  log4r::info(fn_logger, paste(nrow(summary), " tree partition(s) found. Files created: ",
  ...
  log4r::info(fn_logger, paste(nrow(t_summary), " topology partition(s) found. Files created: ", 
...
if (file.exists(fn_nexus) && params$redo == FALSE) {
  ...
  log4r::info(fn_logger, paste("File created: ", params$prefix, ".nex.", sep=""))
}
```
...
if (file.exists(paste(fn_alisim, ".fa", sep = "")) && params$redo == FALSE) {
  ...
  log4r::info(fn_logger, paste("File created: ", params$prefix, ".fa.", sep=""))
}
```