...
for (chr in ls_chr) {
    for (top in ls_topologies) {
        row_temp <- df_topology_count[df_topology_count$wsize=="best" & df_topology_count$chr==chr & df_topology_count$topology==top]

        if (nrow(row_temp) == 0) {
            df_topology_count <- rbind(df_topology_count,
                                       data.table::data.table(chr=chr, wsize="best", topology=top, count=0, count_percentage=0))
        }
    }
}
