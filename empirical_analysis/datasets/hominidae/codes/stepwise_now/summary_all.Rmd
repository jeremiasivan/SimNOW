...
# create outdir
dir_output_prefix <- paste0(params$outdir, "/", prefix, "/")
...
for (chr in ls_chr) {
    ...
    fn_summary <- paste0(params$outdir, "/", params$prefix, ".sumtable")
    ...
    df_output_chr <- foreach (window=df_perwindowsum$name, .combine='rbind') {
        ...
        if (!file.exists(fn_tree)) {
            return(data.table::data.table(window=window, topology=NULL))
        }
