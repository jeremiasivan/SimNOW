    ...
    for (j in 1:nrow(df_colour_scheme)) {
        for (top in ls_uq_tops) {
            ...
            if (ape::all.equal.phylo(top1, top2)) {
                df_topology_colour <- rbind(df_topology_colour, data.table(topology=top, colour=df_colour_scheme$code[j]))
                break
            }
        }
    }
...
df_topology <- data.table::data.table(interval=factor(), chr=factor(), topology=character(), count=numeric(), count_percentage=numeric(), count_highbs=numeric(), count_percentage_highbs=numeric())

# iterate over window spacing
for (interval in ls_window_spacing) {
    # set up temporary data.table
    df_topology_temp <- data.table::data.table(interval=numeric(), chr=factor(), topology=character(), count=numeric(), count_percentage=numeric(), count_highbs=numeric(), count_percentage_highbs=numeric())
