...
f_extract_perwindow_topology <- function(fn_perwindowsum, chr, step, window_size, thread) {
    # open file
...
for (chr in ls_chr) {
    ...
    df_output_chr <- f_extract_perwindow_topology(fn_perwindowsum, chr, best_wsize_step, best_wsize, params$thread)
    ...
    for (step in 1:(length(wsizes)-1)) {
        ...
        if (step == 1) {
            ...
            if (file.exists(fn_perwindowsum)) {
                df_output_wsize <- f_extract_perwindow_topology(fn_perwindowsum, chr, step, long_wsize, params$thread)
        ...
        if (file.exists(fn_perwindowsum)) {
            df_output_wsize <- f_extract_perwindow_topology(fn_perwindowsum, chr, step, short_wsize, params$thread)
