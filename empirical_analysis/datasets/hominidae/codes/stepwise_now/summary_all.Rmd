        ...
        for (idx in ls_idx) {
            ...
            if (file.exists(fn_tree)) {
                ...
                if (is.null(idx_all)) {
                    ...
                    df_topology_chr <- rbind(df_topology_chr, data.table::data.table(window=window, type="highbs", topology=ape::write.tree(init_tree)))
                }
                ...
                if (!is.null(bl) && mean(as.numeric(bl)) >= 95) {
                    tre$node.label <- NULL
                    df_topology_chr <- rbind(df_topology_chr, data.table::data.table(window=window, type="all", topology=ape::write.tree(tre)))
