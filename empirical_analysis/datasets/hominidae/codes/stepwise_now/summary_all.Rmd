...
tiff(filename=fn_conwindows_dist_tiff, units="px", width=1600, height=2800)
print(plot)
dev.off()
```
...
ls_window_spacing <- c(1,2,5,10,20,50,100,200,500,1000)

# set up output data.table
df_topology <- data.table::data.table(interval=numeric(), chr=factor(), topology=character(), count=numeric(), count_percentage=numeric())
...
for (interval in ls_window_spacing) {
    ...
    for (chr in ls_chr) {
        ...
        if (file.exists(fn_filter_toptable_temp) && !params$redo) {
            # combine output files
            df_filter_toptable_temp <- data.table::fread(fn_filter_toptable_temp)
            df_topology_temp <- rbind(df_topology_temp, df_filter_toptable_temp)
        ...
        # set up number of threads
        stepthread <- ifelse(length(ls_idx_filtered) < params$thread, length(ls_idx_filtered), params$thread)
        ...
        df_output_temp <- foreach (idx = ls_idx_filtered, .combine='rbind') %dopar% {
        ...
        # save data.table
        data.table::fwrite(df_output_temp, file=fn_filter_toptable_temp, sep="\t", quote=F)

        # update data.table
        df_topology_temp <- rbind(df_topology_temp, df_output_temp)
    }
...
for (top in sort(unique(df_topology$topology))) {
    ...
    # ggplot2
    plot <- ggplot(df_topology_subset, aes(x=interval, y=count_percentage, group=chr, ymin=0, ymax=1)) +
        geom_line(aes(alpha=0.2), size=4) +
        geom_point(aes(colour=chr), size=15) +
        viridis::scale_color_viridis(discrete = TRUE) + 
        labs(colour="Chromosomes") +
        ggtitle(paste("Distribution of Filtered", topology, "across Window Sizes")) + ylab("Proportion") + xlab("Window Size") +
        guides(alpha="none", size="none", colour=guide_legend(override.aes=list(size=5))) +
        theme(
            plot.title = element_text(hjust = 0.5, size = 50),
            plot.margin = margin(1.25, 1.25, 1.25, 1.25, "cm"),
