...
for (top in sort(unique(df_topology$topology))) {
    ...
    for (chr in ls_chr) {
        if (!chr %in% df_topology_subset$chr) {
            df_topology_subset <- rbind(df_topology_subset, data.table::data.table(interval=unique(df_topology_subset$interval),
                                                                                   chr=rep(chr,length(ls_window_spacing)),
                                                                                   topology=rep(top,length(ls_window_spacing)),
                                                                                   count=rep(0,length(ls_window_spacing)),
                                                                                   count_percentage=rep(0,length(ls_window_spacing))))
        } 
    }
