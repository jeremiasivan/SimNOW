...
# output files
fn_uqtops_dist_tiff <- paste0(dir_output_summary, "all.uqtops_dist.tiff")
fn_conwindows_dist_tiff <- paste0(dir_output_summary, "all.conwindows_dist.tiff")
fn_conwindows_dist_filtered_tiff <- paste0(dir_output_summary, "all.conwindows_dist_filtered.tiff")
```
...
f_plot_perwindow_topology_dist <- function(df_topology_count, topology) {
    plot <- ggplot(df_topology_count, aes(x=as.factor(wsize), y=count_percentage, group=chr)) +
        geom_line(aes(size=1, alpha=0.2)) +
        geom_point(aes(colour=chr, size=20)) +
        ggtitle(paste("Distribution of", topology, "across Window Sizes")) + ylab("Count") + xlab("Window Size") +
...
for (top in sort(unique(df_topology_count$topology))) {
    ...
    print(f_plot_perwindow_topology_dist(df_topology_count_subset, top))
    dev.off()
...
# output files
df_topology_count <- data.table::data.table(chr=character(), wsize=character(), topology=character(), count=numeric(), count_percentage=numeric())

# iterate over chromosomes
for (chr in ls_chr) {
    # output data.table
    df_topology_count_temp <- data.table::data.table(chr=character(), wsize=character(), topology=character(), count=numeric(), count_percentage=numeric())

    # output files
    fn_wintoptable_temp <- paste0(dir_output_prefix, chr, "/", chr, ".all.wintoptable")
    if (file.exists(fn_wintoptable_temp) && !params$redo) {
        # combine output files
        df_wintoptable_temp <- data.table::fread(fn_wintoptable_temp)
        df_topology_count <- rbind(df_topology_count, df_wintoptable_temp)
    ...
    # save data.table
    data.table::fwrite(df_topology_count_temp, file=fn_wintoptable_temp, sep="\t", quote=F)
...
df_topology_count$chr <- fct_inorder(df_topology_count$chr)
...
df_topology_count_long_wsize$wsize <- "best"
