...
f_plot_perwindow_topology_dist <- function(df_topology_count, df_topology_count_avg, topology) {
    ...
    plot <- ggplot(df_topology_count_wsize, aes(x=fct_rev(wsize), y=count_percentage, group=chr)) +
...
tiff(filename=fn_uqtops_dist_tiff, units="px", width=1600, height=2800)
print(plot)
dev.off()
...
for (top in sort(unique(df_topology_count$topology))) {
    ...
    df_topology_count_subset <- df_topology_count_subset %>%
        mutate(chr=ifelse(chr %in% c("chrX","chrY","chrM"),chr,"autosomes"))
