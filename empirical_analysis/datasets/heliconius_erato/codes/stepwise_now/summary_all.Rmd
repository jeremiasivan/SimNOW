...
fn_tops_best_chr <- paste0(dir_summary, "all.best.tops_chr.tsv")
...
ls_topology <- unique(df_topology_count_long$topology)
...
df_topology_count_long <- df_topology_count_long %>%
    filter(!is.na(count) & topology%in%ls_topology) %>%
...
df_topology_consecutive_long <- df_topology_consecutive_long %>%
    filter(topology%in%ls_topology) %>%
