...
df_topology <- data.table(topology=ls_uq_tops, colour=NA, tree=NA)
  ...
  for (j in 1:nrow(df_colour_scheme)) {
    for (top in ls_uq_tops) {
      ...
      if (ape::all.equal.phylo(top1, top2)) {
        df_topology <- rbind(df_topology, data.table(topology=top, colour=df_colour_scheme$code[j], tree=df_colour_scheme$code[j]))
        break
      }
    }
  }
...
plot <- ggplot(df_topology_count_long, aes(x=topology, y=count_percentage))
