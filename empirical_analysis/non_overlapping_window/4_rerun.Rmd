...
if (length(list.files(temp_outdir, pattern='*.fa$', recursive=TRUE)) == numw && !params$redo) {
  ...
  for (i in 1:numw) {
    ...
    fn_out <- paste0(temp_outdir, window_name, "/", window_name, ".fa")
...
for (window in dirs) {
  ...
  if (!file.exists(treefile)) {
    seq_len <- rbind(seq_len, c(window, start+1, stop, length, ""))
  ...
  if (!is.null(params$bootstrap_type) && params$bootstrap_type != "") {
    ...
    if (!is.null(bl) && mean(as.numeric(bl)) >= min_branch_support) {
      tl$node.label <- NULL
      seq_len <- rbind(seq_len, c(window, start+1, stop, length, ape::write.tree(tl)))
    } else {
      seq_len <- rbind(seq_len, c(window, start+1, stop, length, ""))
    }
