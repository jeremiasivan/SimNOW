...
f_topsimsum <- function(df) {
  ...
  if (!all(is.na(df$colour))) {
    plot1 <- plot1 + geom_bar(position="dodge", stat="identity", aes(fill=colour))
    plot2 <- ggplot(subset(df, colour!="#000000"), aes(x=topology, y=count)) + geom_bar(position="dodge", stat="identity", aes(fill=colour))
  } else {
    plot1 <- plot1 + geom_bar(position="dodge", stat="identity") + viridis::scale_fill_viridis(discrete = TRUE)
    plot2 <- ggplot(df[1:8,], aes(x=topology, y=count)) + geom_bar(position="dodge", stat="identity") + viridis::scale_fill_viridis(discrete = TRUE)
  }
...
f_cnsimsum <- function(df1, df2) {
  ...
  if (!all(is.na(df1$colour))) {
    ...
    plot2 <- ggplot(subset(df1, colour!="#000000"), aes(x=count, y=n, fill=colour))
  } else {
