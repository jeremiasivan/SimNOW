...
f_cnsimsum <- function(df1, df2) {
  ...
  df1$tree <- factor(df1$tree, levels=stringr::str_sort(df1$tree, numeric=TRUE))
...
if (params$colour_scheme != "") {
  ...
  for (j in top_topsum) {
    for (k in top_colour) {
      if (ape::all.equal.phylo(ape::unroot(ape::read.tree(text = j)), ape::unroot(ape::read.tree(text = k)))) {
        ...
        df_cnsimsum[topology == j, colour := df_colour$code[topology_idx], tree := df_colour$tree[topology_idx]]
        break
      }
    }
  }
  ...
  df_cnsimsum[is.na(colour), colour := "#000000", tree := "NT"]
}
