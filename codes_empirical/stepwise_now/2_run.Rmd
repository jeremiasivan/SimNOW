...
if (length(list.files(dir_perwindow, pattern='*.fa.log$', recursive=TRUE)) == numw && !params$redo) {
  ...
  # list directory
  ls_windows <- list.dirs(dir_perwindow, full.names=F, recursive=F)

  for (i in ls_windows) {
    fn_input <- paste0(dir_perwindow, "/", i, "/", i, ".fa")

    # generate window trees
    f_iqtree2_single(fn_input, "", FALSE, FALSE, "", "", 0, params$iqtree2dir)
    log4r::info(fn_logger, paste0("File created: ", wsizes[1], "bp per-window alignments for step 1."))
  }
}
...
foreach (i = 1:(length(wsizes)-1)) %dopar% {
  ...
  for (j in ls_windows) {
    fn_input <- paste0(dir_perwindow, "/", j, "/", j, ".fa")
