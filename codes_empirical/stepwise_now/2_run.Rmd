...
if (length(list.files(dir_perwindow, pattern='*.fa$', recursive=TRUE)) == numw && !params$redo) {
  log4r::info(fn_logger, paste0("File found: ", wsizes[1], "bp window alignments for step 1."))
} else {
  # generate window alingments
  f_perwindow_run(dir_perwindow, wsizes[1], numw, s)
  log4r::info(fn_logger, paste0("File created: ", wsizes[1], "bp per-window alignments for step 1."))
}

# check if trees already exist
if (length(list.files(dir_perwindow, pattern='*.fa.log$', recursive=TRUE)) == numw && !params$redo) {
  log4r::info(fn_logger, paste0("File found: ", wsizes[1], "bp window trees for step 1."))
} else {
  # list directory
  ls_windows <- list.files(dir_perwindow, pattern='*.fa$', full.names=T, recursive=T)
  ...
  foreach (w = ls_windows) %dopar% {
    f_iqtree2_single(w, "", FALSE, FALSE, "", "", 0, params$iqtree2dir)
  }

  log4r::info(fn_logger, paste0("File created: ", wsizes[1], "bp per-window trees for step 1."))
  stopCluster(nwcl)
}

# perwindowsum
fn_output <- paste0(dir_perwindow, wsizes[1], ".perwindowsum")
if (file.exists(fn_output)) {
  log4r::info(fn_logger, paste0("File found: ", wsizes[1], "bp window summary file for step 1."))
} else {
  f_perwindow_sum(dir_perwindow, wsizes[1], len_taxa, params$min_informative_sites, fn_output)
}
...
for (i in 1:(length(wsizes)-1)) {
  ...
  if (length(list.files(dir_perwindow, pattern='*.fa$', recursive=TRUE)) == numw && !params$redo) {
    ...
    # generate window alingments
    f_perwindow_run(dir_perwindow, wsizes[i+1], numw, s)
    log4r::info(fn_logger, paste0("File created: ", wsizes[i+1], "bp per-window alignments for step ", i, "."))
  }

  if (length(list.files(dir_perwindow, pattern='*.fa.log$', recursive=TRUE)) == numw && !params$redo) {
    log4r::info(fn_logger, paste0("File found: ", wsizes[i+1], "bp window trees for step ", i, "."))
  } else {
    # list directory
    ls_windows <- list.files(dir_perwindow, pattern='*.fa$', full.names=T, recursive=T)
    ...
    log4r::info(fn_logger, paste0("File created: ", wsizes[i+1], "bp per-window trees for step ", i, "."))
    stopCluster(nwcl)
  }
  
  # perwindowsum
  fn_output <- paste0(dir_perwindow, wsizes[i+1], ".perwindowsum")
  if (file.exists(fn_output)) {
    log4r::info(fn_logger, paste0("File found: ", wsizes[i+1], "bp window summary file for step ", i, "."))
  } else {
    f_perwindow_sum(dir_perwindow, wsizes[i+1], len_taxa, params$min_informative_sites, fn_output)
  }
}
...
foreach (i = 1:(length(wsizes)-1)) %dopar% {
  ...
  if (!all(file.exists(fn_long, fn_short))) {
    log4r::warn(fn_logger, paste0("File not found: .perwindowsum file(s) for step ", i, "."))
  }
