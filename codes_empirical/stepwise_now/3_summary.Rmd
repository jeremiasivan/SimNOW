...
foreach (i = 1:(length(wsizes)-1)) %dopar% {
  ...
  for (j in 1:long_numw) {
    ...
    # iqtree and fasta files for long window
    fn_long_fa <- paste0(dir_long_filtered, long_name, "/", long_name, ".fa")
    fn_long_iqtree <- paste0(dir_long_filtered, long_name, "/", long_name, ".fa.iqtree")
    
    # check if file exists
    if (!file.exists(fn_long_iqtree) && file.exists(fn_long_fa)) {
      if (i == 1) {
    ...
    } else if (all(!file.exists(fn_long_iqtree, fn_long_fa))) {
