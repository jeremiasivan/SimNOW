...
for (i in 2:length(wsizes)-1) {
  ...
  long_ouput <- foreach(j = dirs, .combine=c) %dopar% {
    ...
    if (mean(as.numeric(bl)) >= 80) {
      tl$node.label <- NULL
      return(list(list(aic, ape::write.tree(tl))))
    }

    return(list(list(aic, NULL)))
  }

  long_aic <- sum(long_output[[1]])
  long_tree <- length(subset(long_output[[2]], !is.na(long_output[[2]])))
  ...
  short_aic <- sum(short_output[[1]])
  short_tree <- length(subset(short_output[[2]], !is.na(short_output[[2]])))
