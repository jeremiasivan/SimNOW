...
foreach (i = 2:length(wsizes)-1) %dopar% {
  ...
  if (i == 1) {
    ...
    for (j in 1:numw) {
      fn_out <- f_window_aln(s, start, j*wsizes[i], dir_perwindow, j, wi)

      iqtree2_single(fn_out, params$outgroup, params$set_blmin, params$set_model, params$dna_model, NULL, 0, params$iqtree2dir)
  ...
  for (j in 1:numw) {
    fn_out <- f_window_aln(s, start, j*wsizes[i+1], dir_perwindow, j, wi)
