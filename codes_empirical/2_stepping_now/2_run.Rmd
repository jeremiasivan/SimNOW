...
iqtree2_single <- function(input, outgroup, setblmin, setmodel, dna_model, bootstrap, dir_iqtree2) {
  ...
  if (!is.null(bootstrap) && bootstrap >= 100) {
    iqtree_cmd <- paste(iqtree_cmd, "-b", bootstrap)
  }
...
foreach (i = 2:length(wsizes)-1) %dopar% {
  ...
  if (i == 1) {
    ...
    for (j in 1:numw) {
      ...
      iqtree2_single(fn_out, params$outgroup, params$set_blmin, params$set_model, params$dna_model, 0, params$iqtree2dir)
...
for (i in 2:length(wsizes)-1) {
  ...
  foreach (j = dirs) %dopar% {
    ...
    if (all(file.exists(iqtreefile, treefile))) {
      ...
      iqtree2_single(fastadir, params$outgroup, params$set_blmin, params$set_model, params$dna_model, 1000, params$iqtree2dir)
    }
  }
...
for (i in 2:length(wsizes)-1) {
  ...
  long_ouput <- foreach(j = dirs, .combine=c) %dopar% {
    iqtree_path <- paste0(long_path, j, "/", j, ".fa.iqtree")
    treefile_path <- paste0(long_path, j, "/", j, ".fa.treefile")

    aic <- as.numeric(gsub("^.* ", "", system(paste("grep '^Akaike information criterion'",iqtree_path), intern = T)))
    ...
    if (mean(as.numeric(bl)) >= 80) {
      tl$node.label <- NULL
      return(list(aic, ape::write.tree(tl)))
    }

    return(list(aic, NULL))
  }

  long_aic <- sum(sapply(long_ouput, function(x) x[[1]]))
  long_tree <- unlist(sapply(long_ouput, function(x) {x[[2]]}))
  ...
  short_ouput <- foreach(j = dirs, .combine=c) %dopar% {
  ...
  short_aic <- sum(sapply(short_ouput, function(x) x[[1]]))
  short_tree <- unlist(sapply(short_ouput, function(x) {x[[2]]}))
