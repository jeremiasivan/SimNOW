...
if (!is.null(params$bootstrap_type) && params$bootstrap_type != "") {
  ...
  if (bs_type == "ufboot" && params$bootstrap < 1000) {
    ...
    log4r::error(fn_logger, "Error: invalid type of bootstrap. Please use either 'ufboot' or 'nonparametric'. Exited.")
  }
}
...
foreach (i = 2:length(wsizes)-1) %dopar% {
  ...
  if (i == 1) {
    ...
    for (j in 1:numw) {
      ...
      if (!file.exists(paste0(out[1],".treefile"))) {
        df_long <- rbind(df_long, list(name=out[2],start=start,end=j*wsizes[i],is_informative=FALSE))
      } else {
        df_long <- rbind(df_long, list(name=out[2],start=start,end=j*wsizes[i],is_informative=TRUE))
      }
  ...
  for (j in 1:numw) {
    ...
    iqtree2_single(out[1], params$outgroup, params$set_blmin, params$set_model, params$dna_model, NULL, 0, params$iqtree2dir)
    if (!file.exists(paste0(out[1],".treefile"))) {
      df_short <- rbind(df_short, list(name=out[2],start=start,end=j*wsizes[i+1],is_informative=FALSE))
    } else {
      df_short <- rbind(df_short, list(name=out[2],start=start,end=j*wsizes[i+1],is_informative=TRUE))
    }
...
foreach (i = 2:length(wsizes)-1) %dopar% {
  ...
  ########################################### LONG ############################################
  # check which window is non-informative
  long_noninformative_idx <- c()
  for (j in 1:nrow(df_short)) {
    if (!df_short$is_informative[j]) {
      noninformative_idx <- which(df_long$start <= df_short$start[j] & df_long$end >= df_short$end[j])
      long_noninformative_idx <- c(long_noninformative_idx, noninformative_idx)
    }
  }
  ...
  for (j in long_noninformative_idx) {
    noninformative_idx <- which(df_short$start >= df_long$start[j] & df_short$end <= df_long$end[j])
    short_noninformative_idx <- c(short_noninformative_idx, noninformative_idx)
  }
...
for (i in 2:length(wsizes)-1) {
  ...
  long_output <- foreach(j = dirs, .combine=c) %dopar% {
    ...
    if (!is.null(params$bootstrap_type) && params$bootstrap_type != "") {
      ...
      if (mean(as.numeric(bl)) >= min_branch_support) {
        tl$node.label <- NULL
        return(list(logl, freeparams, ape::write.tree(tl)))
      }
    }
    
    return(list(logl, freeparams, NULL))
  }
