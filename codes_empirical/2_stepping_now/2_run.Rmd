...
for (i in 2:length(wsizes)-1) {
  ...
  window_trees <- readLines(treefile_path)

  long_tree <- foreach(j = window_trees, .combine=c) %dopar% {
    tl <- ape::read.tree(text=j)
    tl$edge.length <- NULL
  ...
  short_tree <- foreach(j = window_trees, .combine=c) %dopar% {
    tl <- ape::read.tree(text=j)
    tl$edge.length <- NULL
