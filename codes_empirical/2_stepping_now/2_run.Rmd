...
for (i in 2:length(wsizes)-1) {
  ...
  long_tree <- NULL
  while (length(trl <- readLines(treefile_path, n=1))) {
    tl <- ape::read.tree(text=trl)
    tl$edge.length <- NULL

    bl <- subset(tl$node.label, tl$node.label != "")
    bl_mean <- mean(as.numeric(bl))

    if (all(as.numeric(bl_mean) >= 95)) {
      tl$node.label <- NULL
      long_tree <- c(long_tree, ape::write.tree(tl))
    }
  }
  close(treefile_path)
  ...
  while (length(trl <- readLines(treefile_path, n=1))) {
    ...
    if (all(as.numeric(bl_mean) >= 95)) {
      tl$node.label <- NULL
      short_tree <- c(short_tree, ape::write.tree(tl))
    }
  }
  close(treefile_path)
