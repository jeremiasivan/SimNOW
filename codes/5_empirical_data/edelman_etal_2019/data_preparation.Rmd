...
ls_filtered <- foreach (i=ls_chr, .combine='c') %dopar% {
  ...
  if (file.exists(output_aln) && !params$redo) {
  ...
  subfasta <- lapply(alignment, function(x){x[idx_leading_gap:idx_trailing_gap]})
  seqinr::write.fasta(sequences=subfasta, names=names(subfasta), file.out=output_aln, nbchar = 100)
