...
f_topsimsum <- function(df, type) {
  ...
  plot2 <- ggplot(df_topsimsum[1:10,], aes(x=topology, y=count, fill=topology)) +
    geom_bar(position="dodge", stat="identity") +
...
# 50kb-window
df_topsimsum_50kb <- df_topsimsum_50kb %>%
  select(topology, count) %>%
  group_by(topology) %>%
  summarise_all(sum) %>%
  mutate(cum.percentage = round(count/sum(count)*100, 3)) %>%
...
df_topsimsum_50kb$topology <- forcats::fct_inorder(df_topsimsum_50kb$topology)

plots <- f_topsimsum(df_topsimsum_50kb, "50kb")
...
plots <- f_topsimsum(df_cnsimsum, df_cnsimsum_ten, "best")
...
# 50kb-window
df_cnsimsum_50kb <- df_cnsimsum_50kb %>%
  group_by(topology, count) %>%
  summarise(n=n())

top_ten_tops <- df_topsimsum_50kb$topology[1:10]
df_cnsimsum_ten_50kb <- subset(df_cnsimsum_50kb, topology %in% top_ten_tops)

plots <- f_topsimsum(df_cnsimsum_50kb, df_cnsimsum_ten_50kb, "50kb")
