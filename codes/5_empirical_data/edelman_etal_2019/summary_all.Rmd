...
df_topsimsum <- df_topsimsum %>%
  ...
  group_by(topology) %>%
  summarise_all(sum) %>%
  mutate(cum.percentage = count / sum(count))

tiff(filename=fn_topsimsum_fig, units="px", width=2880, height=1800)
print(ggplot(df_topsimsum, aes(x=topology, y=count, fill=topology)) +
        geom_bar(position="dodge", stat="identity") +
        geom_text(aes(label=cum.percentage), position=position_dodge(width=0.9), vjust=-0.5) +
...
tiff(filename=fn_cnsimsum_fig, units="px", width=1600, height=2800)
print(ggplot(df_cnsimsum, aes(x=as.factor(count), y=n, fill=topology)) +
        geom_bar(position="dodge", stat="identity") +
        viridis::scale_fill_viridis(discrete = TRUE) +
