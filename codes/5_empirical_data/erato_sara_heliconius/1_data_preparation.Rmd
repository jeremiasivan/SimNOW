...
foreach (i=ls_chr) %dopar% {
  ...
  output_aln <- ifelse(i=="chr_all",
                       paste0(params$data_outdir,"/",i,"/all_concat_filtered.fa"),
                       paste0(params$data_outdir,"/",i,"/fasta/concatenation/",i,"_conca_filtered_.fa"))
  ...
  idx_leading_gap <- head(which(gap_props < params$gaps_threshold), 1)
  idx_trailing_gap <- tail(which(gap_props < params$gaps_threshold), 1)

  # save the filtered FASTA file
  subfasta <- lapply(alignment, function(x){x[idx_leading_gap:idx_trailing_gap]})
  seqinr::write.fasta(sequences=subfasta, names=names(alignment), file.out=output_aln, nbchar = 100)
