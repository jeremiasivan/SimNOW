...
f_delta_ic_acc <- function(df, i, ifc, type) {
  ...
  sdf <- df %>%
    group_by(simulation) %>%
    mutate(max_acc = max(accuracy), min_rmse = min(rmse), min_ifc = min(!!col_ifc)) %>%
    mutate(d_acc = abs(accuracy-max_acc), d_rmse = abs(rmse-min_rmse), d_ifc = abs(!!col_ifc-min_ifc))
  
  plot <- NULL
  if (type == "site") {
    plot <- ggplot(sdf, aes(y=d_acc, x=d_ifc, ymin=0, group=simulation)) +
      ggtitle(paste0("Correlation between \u0394", toupper(ifc), " and \u0394", "Site Classification Accuracy")) +
      xlab(paste0("\u0394", toupper(ifc))) + ylab(paste0("\u0394", "Accuracy (%)"))
  } else if (type == "rmse") {
    plot <- ggplot(sdf, aes(y=d_rmse, x=d_ifc, ymin=0, group=simulation)) +
      ggtitle(paste0("Correlation between \u0394", toupper(ifc), "and \u0394", "RMSE")) +
      xlab(paste0("\u0394", toupper(ifc))) + ylab(paste0("\u0394", "RMSE (%)"))
  }
...
for (i in 6:ncol(simsum)-2) {
  ...
  fn_sitefig <- paste0(ifc_sitedir, params$prefix, ".delta_", ifc, "_site.tiff")
  fn_rmsefig <- paste0(ifc_rmsedir, params$prefix, ".delta_", ifc, "_rmse.tiff")
