...
kable(simsum) %>%
  ...
  scroll_box(width = "100%", height = ifelse(nrow(simsum) > 7, "300px", NULL))
```
...
kable(rank_simsum) %>%
  kable_styling("striped", full_width = T) %>%
  scroll_box(width = "100%", height = ifelse(nrow(rank_simsum) > 7, "300px", NULL))
```
...
kable(best_simsum) %>%
  ...
  scroll_box(width = "100%", height = ifelse(nrow(best_simsum) > 7, "300px", NULL))
```
