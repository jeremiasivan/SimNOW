...
params:
  ...
  # AliSim
  exe_iqtree: "~/iqtree-2.2.2.2-MacOSX/bin/iqtree2"
  alisim_model: "JC"
  alisim_scale: 0.001
...
t_summary <- data.table::data.table()

# check if there are multiple ms_r
if (length(params$ms_r) > 1) {
  # write the msfile
  write.table(paste(params$exe_ms, params$ms_params, "-r rmix", sum(params$ms_l), "-seeds", randseed),
              file=fn_msfile, quote=F, row.names=F, col.names=F)

  for (i in 1:length(params$ms_r)) {
    # run ms
    temp_msfile <- paste0(fn_msfile, i)
    f_ms(params$ms_params, params$ms_r[i], params$ms_l[i], params$prefix, temp_msfile, fn_log, fn_logger, params$redo, randseed, params$exe_ms)

    # extract locus trees and topologies
    temp_mstrees <- f_extract_locus_tree(temp_msfile)
    temp_mstops <- f_extract_locus_topology(temp_mstrees)

    # update the data.frame
    summary <- rbind(summary, temp_mstrees)
    t_summary <- rbind(t_summary, temp_mstops)
  }

  # update the start and stop position
  summary <- summary %>% mutate(start = lag(cumsum(length), default=0)+1, stop = start+length-1)
  t_summary <- t_summary %>% mutate(start = lag(cumsum(length), default=0)+1, stop = start+length-1)
  
} else {
  # run ms
  f_ms(params$ms_params, params$ms_r, params$ms_l, params$prefix, fn_msfile, fn_log, fn_logger, params$redo, randseed, params$exe_ms)
...
alisim_command <- paste(params$exe_iqtree,
                      "--alisim", fn_alisim,
                      "-Q", fn_nexus,
                      "-t", fn_mttrees,
                      "-m", params$alisim_model,
                      "--length", sum(params$ms_l),
                      "--branch-scale", params$alisim_scale,
                      "-seed 1234 -T 1 -af fasta -redo --no-log")
