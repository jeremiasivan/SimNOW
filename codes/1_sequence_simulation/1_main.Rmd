...
if (length(params$ms_r) > 1) {
  # write the msfile
  write.table(paste(params$msdir, params$ms_params, "-r rmix", sum(params$ms_l), "-seeds", randseed),
              file=fn_msfile, quote=F, row.names=F, col.names=F)

  for (i in 1:length(params$ms_r)) {
    # run ms
    temp_msfile <- paste0(fn_msfile, i)
    f_ms(params$ms_params, params$ms_r[i], params$ms_l[i], params$prefix, temp_msfile, fn_log, fn_logger, params$redo, randseed, params$msdir)
  ...
  # run ms
  f_ms(params$ms_params, params$ms_r, params$ms_l, params$prefix, fn_msfile, fn_log, fn_logger, params$redo, randseed, params$msdir)
